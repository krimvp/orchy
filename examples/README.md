...
| [docs-audit](./docs-audit) | `changes: false` on every step, so the workspace proves nothing moved | a repository |
...
three readers and then re-reads the sources, so it costs the most. `triage` and
`decision` read one file and spend the least.
