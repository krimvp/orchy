...
- **`memory.ts` — what a run recovers and stores.** `lines`, `keyOf`,
  `rootKey`, `flowKey`, `scopeKey`, `keyReference`, `referencedKey`,
  `migrateLegacy`, the legacy helper `asKey`, `memoryProblems`, and `MOST`.
  Types: `Entry`, `Storage`.
