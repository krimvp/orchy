...
tool that records it. Across thirty runs against thirteen models on Pi and two
on Claude Code, that was seven of the eight step failures. On a single step it
costs one step. On a fanout it costs the whole run: a flow that judged eleven
clauses of a lease got ten back, and the eleventh took the other ten with it.
